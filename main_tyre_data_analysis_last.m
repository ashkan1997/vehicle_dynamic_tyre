...
FX0_guess = MF96_FX0_vec(TData0.SL,zeros_vec , zeros_vec, FZ0*ones_vec, tyre_coeffs);
...
%P0 = [  1,   2,   1,   0,   0,    1,    0]; 
P0 = [ 1.6,   2,   0,   0,   0,    1,    0];  % --> from paper, pCx1 = 1.6, pEx1 = 0
...
SL_vec = min(KAPPA_vec):0.001:max(KAPPA_vec);
zeros_vec = zeros(size(SL_vec));
ones_vec  = ones(size(SL_vec));

FX0_fz_nom_vec = MF96_FX0_vec(SL_vec,zeros_vec , zeros_vec, FZ0.*ones_vec,tyre_coeffs);

figure()
plot(KAPPA_vec,TData0.FX,'.')
hold on
...
error(4,1) = {'Pure longitudinal slip R2:'};
error(4,2) = {R2};
error(4,3) = {'Pure longitudinal slip RMSE:'};
error(4,4) = {RMSE};
...
P0 = [ -0.02,     0,     0,     0,     0,     0,     0];  % --> from paper, pDy2 = -0.1 < pDx2 = 0
lb = [    -1,    -1,    -1,    -1,    -1,    -1,    -1];
ub = [     1,     1,     1,     1,     1,     1,     1];
...
% Calculate residual:
res_FX0 = resid_pure_Fx_varFz(P_dfz,FX_vec,KAPPA_vec,0 , FZ_vec,tyre_coeffs);
R2      = 1 - res_FX0;
RMSE = sqrt(res_FX0*sum(FX_vec.^2)/length(KAPPA_vec));
error(5,1) = {'Variable load longitudinal slip R2:'};
error(5,2) = {R2};
error(5,3) = {'Variable load longitudinal slip RMSE:'};
error(5,4) = {RMSE};
...
% Calculate residual:
res_FX0 = resid_pure_Fx_varGamma(P_varGamma,FX_vec, KAPPA_vec,GAMMA_vec,tyre_coeffs.FZ0, tyre_coeffs);
R2      = 1 - res_FX0;
RMSE = sqrt(res_FX0*sum(FX_vec.^2)/length(KAPPA_vec));
error(6,1) = {'Variable camber longitudinal slip R2:'};
error(6,2) = {R2};
error(6,3) = {'Variable camber longitudinal slip RMSE:'};
error(6,4) = {RMSE};
...
legend('Raw Data','Fit $\alpha$=0 deg',...
    'Fit $\alpha$=-3 deg','Fit $\alpha$=-6 deg',Location='best')
%title('Combined longitudinal force')


% Calculate residual:
res_FX = resid_comb_Fx(P_comb,FX_comb_vec, KAPPA_comb_vec,ALPHA_comb_vec,0,FZ0,tyre_coeffs,FX0_vec);
R2      = 1 - res_FX;
RMSE = sqrt(res_FX*sum(FX_comb_vec.^2)/length(KAPPA_comb_vec));
error(7,1) = {'Combined longitudinal slip R2:'};
error(7,2) = {R2};
error(7,3) = {'Combined longitudinal slip RMSE:'};
error(7,4) = {RMSE};


%% PLOT GXA -------------------------------------------------------------------------------> DONE

SL_vec = min(KAPPA_comb_vec):1e-4:max(KAPPA_comb_vec);

Gxa = zeros(3,length(KAPPA_comb_vec));
for i=1:length(SL_vec)
    [Gxa(1,i),~,~] = MF96_FXFYCOMB_coeffs(SL_vec(i), mean(SA_0.SA   ), 0, FZ0, tyre_coeffs);
    [Gxa(2,i),~,~] = MF96_FXFYCOMB_coeffs(SL_vec(i), mean(SA_3neg.SA), 0, FZ0, tyre_coeffs);
    [Gxa(3,i),~,~] = MF96_FXFYCOMB_coeffs(SL_vec(i), mean(SA_6neg.SA), 0, FZ0, tyre_coeffs);
end
figure()
plot(SL_vec,Gxa,LineWidth=1.5)
xlabel('$\kappa$[-]')
ylabel('$G_{xa}$')
%title('Weighting function $G_{xa}(\kappa)$')
legend('$\alpha = 0$ deg','$\alpha = -3$ deg','$\alpha = -6$ deg','Location','best')

%% COMBINED LATERAL FY --------------------------------------------------------------------> DONE
...
P0 = [    4.9,   2.2,      0,     1,      0,     0,     0,     0,     0];   % ---> from paper
lb = [      0,     0,     -1,    -1,     -1,    -1,    -1,    -1,    -1];
ub = [     20,    20,      1,     2,      1,    10,     1,    10,    10];
...
% Calculate residual:
res_FY = resid_comb_Fy(P,FY_vec, KAPPA_vec,ALPHA_vec,0,FZ0, tyre_coeffs);
R2      = 1 - res_FY;
RMSE = sqrt(res_FY*sum(FY_vec.^2)/length(KAPPA_vec));
error(8,1) = {'Combined lateral slip R2:'};
error(8,2) = {R2};
error(8,3) = {'Combined lateral slip RMSE:'};
error(8,4) = {RMSE};
...
for i=1:length(SL_vec)
    [~,Gyk(1,i),~] = MF96_FXFYCOMB_coeffs(SL_vec(i), mean(SA_0.SA   ), 0, FZ0, tyre_coeffs);
    [~,Gyk(2,i),~] = MF96_FXFYCOMB_coeffs(SL_vec(i), mean(SA_3neg.SA), 0, FZ0, tyre_coeffs);
    [~,Gyk(3,i),~] = MF96_FXFYCOMB_coeffs(SL_vec(i), mean(SA_6neg.SA), 0, FZ0, tyre_coeffs);
end
figure()
plot(SL_vec,Gyk, LineWidth=2)
xlabel('$\kappa$[-]')
ylabel('$G_{yk}$[-]')
%title('Weighting function $G_{yk}$ as a function of $\kappa$')
legend('$\alpha = 0$ deg','$\alpha = -3$ deg','$\alpha = -6$ deg','Location','best')
...
save(['tyre_no_moment','.mat'],'tyre_coeffs');

save(['R2_err','.mat'],'error');
